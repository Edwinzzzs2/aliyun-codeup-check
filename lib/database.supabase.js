...
function toISOStringOrNull(input) {
  return toBeijingTimestamp(input);
}
...
const AutoMergeDB = {
  ...
  async createTask(task) {
    ...
    const payload = {
      ...
      feishu_config_id: task.feishu_config_id || null,
  ...
  async updateTask(id, task) {
    ...
    if (task.feishu_config_id !== undefined) payload.feishu_config_id = task.feishu_config_id || null;
  ...
  // 普通执行日志排除接口异常；历史空类型日志仍按业务日志展示。
  async getAllLogs(limit = 100, offset = 0, taskNames = null) {
    if (Array.isArray(taskNames) && taskNames.length === 0) return [];
    let query = client
      .from('auto_merge_logs')
      .select('*')
      .or('execution_type.is.null,execution_type.neq.api_exception')
      .order('executed_at', { ascending: false })
      .range(offset, offset + limit - 1);
    if (Array.isArray(taskNames)) query = query.in('task_name', taskNames);
  ...
  // 获取执行日志总数
  async getLogsCount(taskNames = null) {
    if (Array.isArray(taskNames) && taskNames.length === 0) return 0;
    let query = client
      .from('auto_merge_logs')
  ...
  // 异常页只读取接口异常，避免与自动合并、删除分支等业务日志混在一起。
  async getLogsByExecutionType(executionType, limit = 100, offset = 0) {
    const { data, error } = await client
      .from('auto_merge_logs')
      .select('*')
  ...
  async getLogsCountByExecutionType(executionType) {
  ...
  // 创建飞书配置
  async createFeishuConfig(config) {
    const configData = {
      name: config.name,
      ...
      created_at: toISOStringOrNull(new Date()),
      updated_at: toISOStringOrNull(new Date())
    };
  ...
  // 更新飞书配置
  async updateFeishuConfig(id, config) {
    const configData = {
      name: config.name,
      webhook_url: config.webhook_url,
      enabled: config.enabled !== undefined ? config.enabled : true,
  ...
  async createOrUpdateFeishuConfig(config) {
    ...
    if (existing) {
      // 更新现有配置
      return await this.updateFeishuConfig(existing.id, config);
    } else {
      // 创建新配置，如果没有提供名称则使用默认名称
      const configWithName = {
        ...config,
        name: config.name || '默认配置'
      };
      return await this.createFeishuConfig(configWithName);
    }
  },
};
