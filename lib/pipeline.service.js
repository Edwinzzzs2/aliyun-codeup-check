...
function normalizePipelineSteps(run) {
  ...
  return run.stages.flatMap((stage, stageIndex) => {
    const stageInfo = stage?.stageInfo || {};
    const stageKey = stage?.index || stageIndex;
    const steps = [];
    const stageName = stage?.name || stageInfo?.name;
    if (stageName) {
      steps.push({
        id: `stage-${stageKey}`,
        name: String(stageName),
        status: String(stageInfo?.status || stage?.status || 'WAITING').toUpperCase(),
      });
    }

    (Array.isArray(stageInfo?.jobs) ? stageInfo.jobs : []).forEach((job, jobIndex) => {
      if (!job?.name) return;
      steps.push({
        id: `job-${job?.id || job?.jobSign || `${stageKey}-${jobIndex}`}`,
        name: String(job.name),
        status: String(job?.status || 'WAITING').toUpperCase(),
      });
    });
    return steps;
  });
