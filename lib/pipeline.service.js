...
function normalizeStepStatus(value) {
  const status = String(value || 'WAITING').toUpperCase();
  if (['INIT', 'PENDING', 'QUEUED'].includes(status)) return 'WAITING';
  if (status === 'FAILED') return 'FAIL';
  return status;
}

function normalizePipelineSteps(run) {
  if (!Array.isArray(run?.stages)) return [];

  return run.stages.map((stage, stageIndex) => {
    const stageInfo = stage?.stageInfo || {};
    const firstJob = Array.isArray(stageInfo?.jobs) ? stageInfo.jobs[0] : null;
    const stageName = stage?.name || stageInfo?.name || firstJob?.name;
    if (!stageName) return null;

    // Flow 的 jobs 是阶段内部执行明细，不是下一流程节点；卡片只按顶层 stages 绘制。
    return {
      id: `stage-${stage?.index ?? stageIndex}`,
      name: String(stageName).trim(),
      status: normalizeStepStatus(stageInfo?.status || stage?.status || firstJob?.status),
    };
  }).filter(Boolean);
}
...
function normalizePipelineRun(data) {
  ...
  return {
    ...
    durationSeconds: startTimestamp && endTimestamp
      ...
      : null,
    // 只保留页面绘制步骤状态所需的精简字段，避免把完整运行详情写入本地数据库。
    steps: normalizePipelineSteps(run),
  };
}
