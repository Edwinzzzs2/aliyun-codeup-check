...
const PipelineDB = {
  ...
  async getCompactLogs({ taskId = null, historyLimit = 50 } = {}) {
    ...
    let scheduledQuery = client
      ...
      .limit(500);
    if (taskId) {
      historyQuery = historyQuery.eq('task_id', taskId);
      scheduledQuery = scheduledQuery.eq('task_id', taskId);
    }

    const [{ data: history, error: historyError }, { data: scheduled, error: scheduledError }] = await Promise.all([
      historyQuery,
      scheduledQuery,
    ]);
    ...
    // 无变化的自动检测每个任务只展示最新一条；真正触发过流水线的记录始终保留在历史中。
    const latestScheduledByTask = new Map();
    (scheduled || []).forEach((log) => {
      const key = String(log.task_id || log.task_name || 'unknown');
      if (!latestScheduledByTask.has(key)) latestScheduledByTask.set(key, log);
    });

    return [...(history || []), ...latestScheduledByTask.values()]
