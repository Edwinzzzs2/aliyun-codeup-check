'use strict';

const { supabase } = require('./supabase');
const { toBeijingTimestamp } = require('./time');
const { getNextRun } = require('./pipeline.schedule');

const client = supabase();

const LOG_SEARCH_LABELS = {
  ready: '已建基线',
  success: '已触发',
  no_change: '无变更',
  failed: '失败',
  skipped: '已跳过',
  RUNNING: '运行中',
  SUCCESS: '运行成功',
  FAIL: '运行失败',
  CANCELED: '已取消',
  WAITING: '等待中',
  scheduled: '自动检测',
  manual: '手动运行',
  setup: '建立基线',
  manual_check: '手动检测',
};

function parseTask(task) {
  if (!task) return task;

  try {
    return { ...task, extra_envs: JSON.parse(task.extra_envs || '{}') };
  } catch {
    return { ...task, extra_envs: {} };
  }
}

function taskPayload(task, partial = false) {
  const payload = {};
  const assign = (field, value, originalValue = value) => {
    if (!partial || originalValue !== undefined) payload[field] = value;
  };

  assign('name', task.name);
  assign('pipeline_id', task.pipeline_id ? String(task.pipeline_id) : task.pipeline_id);
  assign('organization_id', task.organization_id);
  assign('repository_id', task.repository_id ? String(task.repository_id) : task.repository_id);
  assign('repository_name', task.repository_name || null, task.repository_name);
  assign('repository_url', task.repository_url);
  assign('branch_name', task.branch_name);
  assign('interval_minutes', task.interval_minutes);
  assign('weekday_interval_minutes', task.weekday_interval_minutes);
  assign('weekend_interval_minutes', task.weekend_interval_minutes);
  assign('enabled', task.enabled);
  assign('force_update', task.force_update);
  assign(
    'extra_envs',
    task.extra_envs === undefined
      ? undefined
      : JSON.stringify(task.extra_envs || {})
  );
  assign('last_commit_id', task.last_commit_id || null, task.last_commit_id);

  return payload;
}

const PipelineDB = {
  async getAllTasks() {
    const { data, error } = await client
      .from('pipeline_tasks')
      .select('*')
      .order('created_at', { ascending: false });
    if (error) throw new Error(error.message);
    return (data || []).map(parseTask);
  },

  async getTaskById(id) {
    const { data, error } = await client
      .from('pipeline_tasks')
      .select('*')
      .eq('id', id)
      .maybeSingle();
    if (error) throw new Error(error.message);
    return parseTask(data);
  },

  async getDueTasks() {
    const { data, error } = await client
      .from('pipeline_tasks')
      .select('*')
      .eq('enabled', true)
      .lte('next_run', toBeijingTimestamp(new Date()));
    if (error) throw new Error(error.message);
    return (data || []).map(parseTask);
  },

  async createTask(task) {
    const now = new Date();
    const nextRun = getNextRun(task, now);
    const payload = {
      ...taskPayload(task),
      enabled: task.enabled !== false,
      force_update: task.force_update !== false,
      last_status: 'ready',
      last_message: '已记录分支基线，等待代码变更',
      next_run: toBeijingTimestamp(nextRun),
      created_at: toBeijingTimestamp(now),
      updated_at: toBeijingTimestamp(now),
    };
    const { data, error } = await client
      .from('pipeline_tasks')
      .insert(payload)
      .select('*')
      .single();
    if (error) throw new Error(error.message);
    return parseTask(data);
  },

  async updateTask(id, task) {
    const now = new Date();
    const payload = {
      ...taskPayload(task, true),
      updated_at: toBeijingTimestamp(now),
    };
    const scheduleChanged = [
      task.interval_minutes,
      task.weekday_interval_minutes,
      task.weekend_interval_minutes,
    ].some((value) => value !== undefined);
    if (scheduleChanged) payload.next_run = toBeijingTimestamp(getNextRun(task, now));
    const { data, error } = await client
      .from('pipeline_tasks')
      .update(payload)
      .eq('id', id)
      .select('*')
      .maybeSingle();
    if (error) throw new Error(error.message);
    return parseTask(data);
  },

  async deleteTask(id) {
    const { error } = await client.from('pipeline_tasks').delete().eq('id', id);
    if (error) throw new Error(error.message);
  },

  async updateRunState(id, state) {
    const payload = {
      last_run: toBeijingTimestamp(state.lastRun || new Date()),
      next_run: toBeijingTimestamp(state.nextRun),
      last_status: state.status,
      last_message: state.message,
      updated_at: toBeijingTimestamp(new Date()),
    };
    if (state.commitId !== undefined) payload.last_commit_id = state.commitId;
    if (state.pipelineRunId !== undefined) {
      payload.last_pipeline_run_id = state.pipelineRunId == null
        ? null
        : String(state.pipelineRunId);
    }

    const { error } = await client
      .from('pipeline_tasks')
      .update(payload)
      .eq('id', id);
    if (error) throw new Error(error.message);
  },

  async logExecution(log) {
    const payload = {
      task_id: log.taskId,
      task_name: log.taskName,
      status: log.status,
      message: log.message,
      commit_id: log.commitId || null,
      commit_message: log.commitMessage || null,
      pipeline_id: log.pipelineId == null ? null : String(log.pipelineId),
      pipeline_run_id: log.pipelineRunId == null ? null : String(log.pipelineRunId),
      pipeline_status: log.pipelineStatus || null,
      pipeline_start_time: toBeijingTimestamp(log.pipelineStartTime),
      pipeline_end_time: toBeijingTimestamp(log.pipelineEndTime),
      pipeline_duration_seconds: log.pipelineDurationSeconds ?? null,
      pipeline_steps: Array.isArray(log.pipelineSteps) ? log.pipelineSteps : null,
      trigger_type: log.triggerType || 'scheduled',
      request_data: log.requestData ? JSON.stringify(log.requestData) : null,
      response_data: log.responseData ? JSON.stringify(log.responseData) : null,
      error_details: log.errorDetails || null,
      executed_at: toBeijingTimestamp(new Date()),
    };
    const { error } = await client.from('pipeline_logs').insert(payload);
    if (error) throw new Error(error.message);
  },

  async updateLogPipelineRun(id, run) {
    const payload = {
      pipeline_status: run.status || null,
      pipeline_start_time: toBeijingTimestamp(run.startTime),
      pipeline_end_time: toBeijingTimestamp(run.endTime),
      pipeline_duration_seconds: run.durationSeconds ?? null,
      pipeline_steps: Array.isArray(run.steps) ? run.steps : null,
    };
    const { error } = await client
      .from('pipeline_logs')
      .update(payload)
      .eq('id', id);
    if (error) throw new Error(error.message);
  },

  async updateLogCommitMessage(id, commitMessage) {
    const { error } = await client
      .from('pipeline_logs')
      .update({ commit_message: commitMessage })
      .eq('id', id);
    if (error) throw new Error(error.message);
  },

  async getLogs({ taskId = null, taskIds = null, limit = 50, offset = 0 } = {}) {
    if (Array.isArray(taskIds) && taskIds.length === 0) return [];
    let query = client
      .from('pipeline_logs')
      .select('*')
      .order('executed_at', { ascending: false })
      .range(offset, offset + limit - 1);
    if (taskId) query = query.eq('task_id', taskId);
    else if (Array.isArray(taskIds)) query = query.in('task_id', taskIds);
    const { data, error } = await query;
    if (error) throw new Error(error.message);
    return data || [];
  },

  async getLogsWithPipelineRuns(limit = 100, taskIds = null) {
    if (Array.isArray(taskIds) && taskIds.length === 0) return [];
    let query = client
      .from('pipeline_logs')
      .select('*')
      .not('pipeline_run_id', 'is', null)
      .order('executed_at', { ascending: false })
      .limit(limit);
    if (Array.isArray(taskIds)) query = query.in('task_id', taskIds);
    const { data, error } = await query;
    if (error) throw new Error(error.message);
    return data || [];
  },

  async getCompactLogs({ taskId = null, taskIds = null, limit = 20, offset = 0, search = '' } = {}) {
    if (Array.isArray(taskIds) && taskIds.length === 0) {
      return { logs: [], totalCount: 0 };
    }
    const history = [];
    const batchSize = 1000;

    // 历史流水线和人工操作数量增长较慢，分批取全后再与“最新自动检测”统一搜索、排序和分页。
    for (let batchOffset = 0; ; batchOffset += batchSize) {
      let historyQuery = client
        .from('pipeline_logs')
        .select('*')
        .or('trigger_type.neq.scheduled,pipeline_run_id.not.is.null')
        .order('executed_at', { ascending: false })
        .range(batchOffset, batchOffset + batchSize - 1);
      if (taskId) historyQuery = historyQuery.eq('task_id', taskId);
      else if (Array.isArray(taskIds)) historyQuery = historyQuery.in('task_id', taskIds);
      const { data, error } = await historyQuery;
      if (error) throw new Error(error.message);
      history.push(...(data || []));
      if (!data || data.length < batchSize) break;
    }

    let taskQuery = client.from('pipeline_tasks').select('id');
    if (taskId) taskQuery = taskQuery.eq('id', taskId);
    else if (Array.isArray(taskIds)) taskQuery = taskQuery.in('id', taskIds);
    const { data: tasks, error: taskError } = await taskQuery;
    if (taskError) throw new Error(taskError.message);
    const latestScheduledResults = await Promise.all((tasks || []).map(async (task) => {
      const { data, error } = await client
        .from('pipeline_logs')
        .select('*')
        .eq('task_id', task.id)
        .eq('trigger_type', 'scheduled')
        .is('pipeline_run_id', null)
        .order('executed_at', { ascending: false })
        .limit(1)
        .maybeSingle();
      if (error) throw new Error(error.message);
      return data;
    }));

    const normalizedSearch = search.trim().toLocaleLowerCase();
    const compactLogs = [...history, ...latestScheduledResults.filter(Boolean)]
      .filter((log) => {
        if (!normalizedSearch) return true;
        return [
          log.task_name,
          log.message,
          log.commit_id,
          log.commit_message,
          log.pipeline_id,
          log.pipeline_run_id,
          log.pipeline_status,
          log.status,
          log.trigger_type,
          LOG_SEARCH_LABELS[log.pipeline_status],
          LOG_SEARCH_LABELS[log.status],
          LOG_SEARCH_LABELS[log.trigger_type],
        ].filter(Boolean).join(' ').toLocaleLowerCase().includes(normalizedSearch);
      })
      .sort((left, right) => new Date(right.executed_at) - new Date(left.executed_at));

    return {
      logs: compactLogs.slice(offset, offset + limit),
      totalCount: compactLogs.length,
    };
  },

  async getLogsCount(taskId = null, taskIds = null) {
    if (Array.isArray(taskIds) && taskIds.length === 0) return 0;
    let query = client
      .from('pipeline_logs')
      .select('*', { count: 'exact', head: true });
    if (taskId) query = query.eq('task_id', taskId);
    else if (Array.isArray(taskIds)) query = query.in('task_id', taskIds);
    const { count, error } = await query;
    if (error) throw new Error(error.message);
    return count || 0;
  },
};

module.exports = { PipelineDB, toBeijingTimestamp };
